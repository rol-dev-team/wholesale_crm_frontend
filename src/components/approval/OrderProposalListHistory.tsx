...
export default function OrderProposalListHistory({ filters }: OrderProposalListHistoryProps) {
  ...
  const fetchHistory = async (page = 1) => {
    ...
    try {
      ...
      const resolvedTotal = meta?.total ?? data.length ?? 0;
      let resolvedLast;
      if (meta?.last_page) {
        resolvedLast = meta.last_page;
      } else if (meta?.total) {
        resolvedLast = Math.ceil(meta.total / ITEMS_PER_PAGE);
      } else {
        // no meta, assume more pages if current page is full
        if (data.length === ITEMS_PER_PAGE) {
          resolvedLast = currentPage + 1;
        } else {
          resolvedLast = currentPage;
        }
      }
      resolvedLast = resolvedLast ?? 1;
      const resolvedCurrent = meta?.current_page ?? page;

      setCurrentPage(resolvedCurrent);
      setTotalPages(resolvedLast);
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <>
          ...
          {/*
            Pagination guard:
            - Always renders when totalItems > 0
            - Passes totalPages fallback of 1 to prevent NaN/0 edge cases
            - The onPageChange guard prevents out-of-range calls
          */}
          {totalItems > 0 && (
            <AppPagination
              currentPage={currentPage}
