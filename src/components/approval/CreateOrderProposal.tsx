...
export default function CreateOrderProposal({ proposal }: Props) {
  ...
  const calculateInvoiceDifference = (totalAmount?: string, currentInvoice?: string | number) => {
    if (!totalAmount || totalAmount === '' || currentInvoice === '-' || currentInvoice === undefined) {
      return undefined;
    }

    const total = parseFloat(totalAmount);
    const invoice = typeof currentInvoice === 'number' ? currentInvoice : parseFloat(String(currentInvoice));
    ...
    setRows((prev) =>
      prev.map((r) => {
        if (r.product === product) {
          ...
          const difference = calculateInvoiceDifference(updated.total_amount, updated.current_invoice);
