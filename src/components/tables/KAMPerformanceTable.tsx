...
export const KAMPerformanceTable: React.FC<Props> = ({
  ...
  <Table 
    ...
    containerClassName="h-[450px] overflow-auto relative" 
  ...
  >
    <TableHeader className="bg-slate-100">
      ...
      <TableRow className="hover:bg-transparent border-none">
        ...
        <TableHead 
          className="text-center border-r border-b text-[10px] uppercase bg-slate-200 sticky top-[40px] right-[240px] z-[90] min-w-[120px] h-[40px]"
        >
          Total Invoice
        </TableHead>
        <TableHead 
          className="text-center border-r border-b text-[10px] uppercase bg-slate-200 sticky top-[40px] right-[120px] z-[90] min-w-[120px] h-[40px]"
        >
          Total Target
        </TableHead>
        <TableHead 
          className="text-center border-b text-[10px] uppercase bg-slate-200 sticky top-[40px] right-0 z-[90] min-w-[120px] h-[40px]"
        >
          Total Achievement
        </TableHead>
      </TableRow>
    ...
    <TableBody>
      {kamPerformanceData.map((kam) => (
        <TableRow key={kam.id} className="hover:bg-muted/20">
          <TableCell 
            className="font-medium border-r border-b bg-white sticky left-0 z-20"
          >
            {kam.name}
          </TableCell>
          ...
          {kam.periodStats.map((stat, i) => (
            <React.Fragment key={i}>
              ...
              <TableCell className="text-center border-r border-b text-sm text-slate-600">{stat.targetAmount > 0 ? `${((stat.successAchieved / stat.targetAmount) * 100).toFixed(2)}%` : '0%'}</TableCell>
