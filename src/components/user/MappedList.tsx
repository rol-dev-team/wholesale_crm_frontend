      ...
      <TableBody>
        ...
        {mappings.map((m, idx) => (
          <TableRow key={idx}>
            ...
            <TableCell>
              {m.groups.map((g) => groupOptions.find((opt) => opt.id === g)?.label).join(', ')}
            </TableCell>
            <TableCell>
              {m.teams.map((t) => teamOptions.find((opt) => opt.id === t)?.label).join(', ')}
            </TableCell>
            <TableCell>
              {m.kams.map((k) => systemUsers.find((u) => u.id === k)?.name).join(', ')}
            </TableCell>
