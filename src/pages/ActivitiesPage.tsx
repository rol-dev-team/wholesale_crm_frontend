...
const matchesType =
typeFilters.length === 0 || typeFilters.includes(a.type);
...
if (isSupervisor && kamFilter !== "all" && a.kamId !== kamFilter)
return false;

if (isManagement) {
if (divisionFilter !== "all" && kam?.division !== divisionFilter) return false;
if (supervisorFilter !== "all" && kam?.reportingTo !== supervisorFilter) return false;
if (kamFilter !== "all" && a.kamId !== kamFilter) return false;
}
...
const filters = [
  ...(isSupervisor || isManagement
    ? [{
        ...
        options: [{ label: "All Divisions", value: "all" }, ...divisions.map(d => ({ label: d, value: d }))],
      }]
    : []),
  ...
  ...(isManagement
    ? [{
        ...
        options: [{ label: "All Supervisors", value: "all" }, ...supervisors.map(s => ({ label: s, value: s }))],
      }]
    : []),
  ...
  ...(isSupervisor || isManagement
    ? [{
        ...
        options: [{ label: "All KAMs", value: "all" }, ...kams.map(k => ({ label: k.name, value: k.id }))],
      }]
    : []),

  {
    ...
    label: "Activity Type",
    value: typeFilters,
    onChange: (v: ActivityType[]) => setTypeFilters(v),
    options: activityTypeOptions,
  },
  {
    type: "search-select" as const,
...
<ActivityList
      activities={paginatedActivities}
      clients={clients}
      showClientInfo onEdit={function (activity: Activity): void {
        throw new Error("Function not implemented.");
      } } onComplete={function (activityId: string, message: string): void {
        throw new Error("Function not implemented.");
      } }/>
