  ...
  const handleSaveUserAccess = (data: UserAccessFormValues) => {
    if (editingUserAccessIndex !== null) {
      setUserAccesses((prev) =>
        prev.map((ua, idx) => (idx === editingUserAccessIndex ? data : ua))
      );
        ...
        <TabsContent value="teams">
          <Card>
            <CardHeader className="flex justify-between items-center">
              ...
              <Button
                onClick={() => setShowTeamForm((p) => !p)}
                variant={showTeamForm ? 'destructive' : 'default'}
              >
                    ...
                    teams.map((t, i) => (
                      <tr key={i}>
                        ...
                        <td className="border p-2">
                          {t.kams
                            .map((id) => systemUserList.find((u) => u.id === id)?.fullName)
                            .join(', ')}
                        </td>
                        <td className="border p-2">
                          {t.supervisors
          ...
          <Card>
            <CardHeader className="flex justify-between items-center">
              ...
              <Button
                ...
                variant={showGroupForm ? 'destructive' : 'default'}
              >
                  ...
                  <div key={i} className="border p-3 rounded-md flex justify-between">
                    <div>
                      ...
                      <div>
                        ...
                        {g.teams
                          .map((t) => teamOptions.find((opt) => opt.value === t)?.label)
                          .join(', ')}
                      </div>
                      <div>
                        <span className="font-medium">Supervisors:</span>{' '}
                        {g.supervisors
                          .map((s) => supervisorOptions.find((opt) => opt.value === s)?.label)
                          .join(', ')}
                      </div>
                    </div>
                    <div className="flex gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="destructive"
                        onClick={() => handleCreateOrUpdateGroup({ ...g, groupName: 'DELETE' }, i)}
                      >
                        <Trash className="h-4 w-4" />
                      </Button>
                    </div>
                ...
                <tbody>
                  {userAccesses.length === 0 ? (
                    ...
                    userAccesses.map((ua, i) => (
                      <tr key={i}>
                        <td className="border p-2">
                          {systemUserList.find((u) => u.id === ua.userId)?.fullName}
                        </td>
                        <td className="border p-2">
                          {ua.groups
                            .map((gId) => userAccessGroupOptions.find((g) => g.id === gId)?.label)
                            .join(', ')}
                        </td>
                        <td className="border p-2">
                          {ua.teams
                            .map((tId) => userAccessTeamOptions.find((t) => t.id === tId)?.label)
                            .join(', ')}
                        </td>
                        <td className="border p-2">
                          {ua.kams
                            .map((kId) => systemUserList.find((u) => u.id === kId)?.fullName)
                            .join(', ')}
                        </td>
                        <td className="border p-2 flex gap-2">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleEditUserAccess(i)}
                          >
                            <Edit2 className="h-4 w-4" />
                          </Button>
                        </td>
        ...
        >
          <Card>
            ...
            <CardContent className="overflow-visible">
              <SystemUserList users={systemUserList} onEdit={handleEditSystemUser} />
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card>
            ...
            <CardContent>
              <UserMappingForm
                ...
                initialValues={
                  editingUserMappingIndex !== null
                    ? userMappings[editingUserMappingIndex]
                    : undefined
                }
              />
            </CardContent>
